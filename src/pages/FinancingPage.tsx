...
import { Banknote, Calculator, MessageCircle } from "lucide-react";
...
const routeApi = getRouteApi("/financiamento");
...
export function FinancingPage() {
  const { valor: valorParam } = routeApi.useSearch();
  const { openContact } = useContact();

  const initialValor = valorParam ?? 100000;
  const [state, setState] = React.useState<SimState>({
    valor: initialValor,
    entrada: Math.round(initialValor * 0.3),
    parcelas: 48,
    taxa: 1.49,
  });
    ...
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_340px]">
        ...
        <Card className="gap-0 p-6">
          ...
          <Button className="mt-5 w-full" onClick={() => openContact()}>
            <MessageCircle />
            Falar com um consultor
          </Button>
          <p className="mt-3 flex items-start gap-1.5 text-xs text-muted-foreground">
            <Banknote className="mt-0.5 size-3.5 shrink-0" />
            Aprovação de crédito em até 24h, com as principais financeiras.
          </p>
        </Card>
      </div>
    </main>
