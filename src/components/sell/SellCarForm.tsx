...
/** Monta o body do POST /showroom/{slug}/sell-offers a partir do wizard. */
function toPayload(values: SellFormValues): SellOfferPayload {
  const notes = [
    `Câmbio: ${values.cambio}`,
    `Estado geral: ${ESTADO_LABELS[values.estado] ?? values.estado}`,
    `Único dono: ${values.unicoDono === "sim" ? "sim" : "não"}`,
    `Documentação: ${DOC_LABELS[values.docOk] ?? values.docOk}`,
    `Cidade: ${values.cidade}`,
  ].join(" · ");

  const mileage = Number(values.km.replace(/\D/g, ""));
  const askingPrice = values.precoDesejado
    ? Number(values.precoDesejado.replace(/\D/g, ""))
    : undefined;
...
export function SellCarForm({ whatsapp }: { whatsapp: string | null }) {
      ...
      <main className="mx-auto max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
        <Card className="items-center gap-0 p-9 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            ...
            em contato em até 24h úteis com uma proposta inicial.
          </p>
        ...
        <div className="flex flex-col gap-5">
          ...
          {step === 2 && (
            <>
              ...
              <Field label="Quilometragem aproximada" htmlFor="sf-km" error={errors.km?.message}>
                <Input
                  id="sf-km"
                  inputMode="numeric"
          ...
          {step === 3 && (
            <>
              ...
              />
              <Field
                label="Quanto você espera pelo veículo?"
                htmlFor="sf-preco"
                optional
                error={errors.precoDesejado?.message}
              >
                <Input
                  id="sf-preco"
                  inputMode="numeric"
          ...
          {step === 4 && (
            <>
              ...
              <Field label="Telefone (WhatsApp)" htmlFor="sf-tel" error={errors.telefone?.message}>
                <Input
                  id="sf-tel"
                  inputMode="tel"
